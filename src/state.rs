...
enum State {
    ...
    // The kernel transition to this state when jumping to kernel_main()
    SingleCoreMain,

    // The kernel transitions to this state when it bts the secondary cores, aka switchs
    // execution mode to symmetric multiprocessing (SMP).
    MultiCoreMain,
}
